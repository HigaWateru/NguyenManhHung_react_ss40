...
import {Table, Button, Modal, Form, Input, Upload, Pagination, Select} from "antd";
...
const ProductTable = () => {
  ...
  const columns: ProductColumn[] = [
    ...
    {title: "Hình ảnh", dataIndex: "image", key: "image", render: (text: string) => (
        <img src={text} alt="product" className="w-20 h-20 object-cover" />
      ),
    },
    {title: "Trạng thái", dataIndex: "status", key: "status", },
    {title: "Thao tác", key: "actions", render: (_text: string, record?: Product) => (
        <div className="flex space-x-2">
          <Button type="primary" onClick={() => record && handleEdit(record)}>Sửa</Button>
          <Button danger onClick={() => record && handleDelete(record.id)}>Xóa</Button>
        </div>
      ),
    },
  ];
  ...
  const handleDelete = (id: string) => setProducts(products.filter((product) => product.id !== id))
  ...
  const handleSearch = (e: SearchEvent): void => setSearchText(e.target.value)
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between mb-4">
        <Button type="primary" onClick={handleAddNew}>Thêm mới sản phẩm</Button>
      </div>
